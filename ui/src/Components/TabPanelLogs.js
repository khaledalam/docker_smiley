...
function TabPanelLogs() {
      ...
      setContainersData(Object.keys(res?.data)?.map((containerKey, index) => {
        ...
        return {
          ...
          subRows: res?.data[containerKey].map((data, index) => {
            return {
              description: data?.Names[0],
              subRows: Object.values(data?.Logs).map((env, index) => {
                return {
                  description: env?.Line,
                }
              })
            }
          })
  ...
  return <>
          <div className="input-group">
            <span className="input-group-text" id="basic-addon1">substring of logs</span>
